...
export default function PostLayout({ frontMatter, authorDetails, next, prev, children }) {
  ...
  const postUrl = `${siteMetadata.siteUrl}/artigos/${slug}`
  return (
    <SectionContainer>
      <BlogSEO
        url={`${siteMetadata.siteUrl}/artigos/${slug}`}
            ...
            <div className="space-y-1 text-center">
              ...
              <div className="flex justify-center gap-5 py-4">
                <span className="flex items-center gap-1.5">
                  ...
                  {readingTime.words} palavras
                </span>
                <span className="flex items-center gap-1.5">
                  <HiOutlineClock className="h-5 w-5" />
                  {readingTime.text.replace('read', 'de leitura')}
                </span>
                {/* temporarily hiding the page view count */}
                {/* <span className="flex items-center gap-1.5">
                    ...
                    {prev && (
                      <div>
                        ...
                        <div className="text-primary-500 hover:text-primary-600 dark:hover:text-primary-400">
                          <Link href={`/artigos/${prev.slug}`}>{prev.title}</Link>
                        </div>
                      </div>
                    )}
                    {next && (
                      <div>
                        ...
                        <div className="text-primary-500 hover:text-primary-600 dark:hover:text-primary-400">
                          <Link href={`/artigos/${next.slug}`}>{next.title}</Link>
                        </div>
                      </div>
                    )}
                  </div>
