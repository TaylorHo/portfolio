	...
	return (
		<>
			<div className="mx-auto max-w-6xl divide-y divide-gray-400">
				...
				<ul>
					{!filteredBlogPosts.length && "Nenhum artigo encontrado."}
					{displayPosts.map((frontMatter) => {
						const { slug, date, title, summary, tags } = frontMatter;
						return (
							<li
								key={slug}
								className="py-6 group flex bg-transparent bg-opacity-20 px-2 transition duration-100 hover:scale-105 hover:rounded-xl hover:bg-gray-100 dark:hover:bg-gray-800 cursor-pointer"
								onClick={() => {
									push(`/artigos/${slug}`);
								}}
							>
